"""[안 쓴다] 건축HUB 전국 마트에서 서울만 뽑던 도구.

## 2026-09-01 — 할 일이 없어졌다

전국본을 받지 않는다. scripts/hub/download_seoul.py 가 **처음부터 서울만** 받는다
(시군구 단위로 끊어 55장 × 25구). 뽑을 전국본이 없으니 이 스크립트도 돌 일이 없다.

전국본을 다시 받는 날이 오면 되살리면 된다. 그래서 지우지 않고 둔다.
