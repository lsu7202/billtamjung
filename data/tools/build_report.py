...
WARN_FIX = 0.01       # 우리가 바꾼 값이 이보다 많으면 경고
...
class Report:
    ...
    def note_odd(self, why, key=None, value=None):
    ...
    def finish(self, quiet=False):
        ...
        f_tot = sum(self.fixes.values()) + sum(self.nulls.values())
        r = max(self.n_read, 1)
        verdict, reasons = "정상", []
        if self.n_read == 0:
            verdict = "중단"; reasons.append("원본을 한 줄도 못 읽었다")
        elif self.n_write < self.n_read * STOP_YIELD:
            verdict = "중단"
            reasons.append(f"낸 줄 {self.n_write:,} 이 읽은 줄 {self.n_read:,} 의 "
                           f"{STOP_YIELD:.0%} 미만이다")
        else:
