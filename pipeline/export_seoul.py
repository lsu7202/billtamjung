    ...
    with open(args.out, "w", newline="") as f:
        ...
        for row in cur:
            ...
            rowvals = [
                ...
                row[ci["지상층수"]] or "",
                # **지하 0층은 「지하가 없다」는 사실이다.** `or ""` 로 두면 0 이 빈칸이 되고
                # 로더의 NULLIF 가 NULL 로 만들어 「모름」과 구별이 없어진다 — 585,731동 중
                # 214,993동(36.7%)이 그렇게 둔갑해 있었다(2026-09-07 실측).
                # 지상층수는 그대로 둔다: 지상 0층인 건물은 없으므로 원본 0 은 「모름」이 맞다.
                (row[ci["지하층수"]] if row[ci["지하층수"]] is not None else ""),
