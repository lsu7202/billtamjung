...
NAMES=(  ui                     data                       screen                        mirror                     build )
DESCS=(  "UI 규칙(네모버튼·설명글씨)"  "DB 값·불변식 26가지"          "화면이 DB 값을 맞게 읽는가"       "한 사실이 여러 표에서 같은가"   "빌드 산출물이 맞는가" )
# 무엇이 바뀌면 도는가
PATS=(   '^frontend/'
...
for i in "${!NAMES[@]}"; do
  ...
  echo; echo "── $name · $desc"
  case "$name" in
    data|mirror|build) need_db || { echo "   ⏭ 건너뜀 — DB(55432)가 안 떠 있습니다"; SKIP=$((SKIP+1)); continue; };;
