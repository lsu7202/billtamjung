...
GROUPS: list[tuple[str, list]] = [
  ...
  ("[정합] 추정가 — 화면끼리 같은가", [
    # master.building_sale_est — **배치를 다시 돌리면 움직인다**(실거래·지수가 최신으로 갱신됨).
    # 2026-09-05 파이프라인 재실행으로 1023억 → 1024억. 값 자체보다 **화면끼리 같은가**가 요점이다.
    ("/buildings/1024123619", "건물 상세 · 1024억", "빌탐정 추정가",
     ["1024억", "2.51%"],
  ...
  ("[정합] 주변 소식 — area_event 를 읽는다", [
    # master.area_event 에서 삼성동 78 둘레 700m.
    # 입지 탭 안이라 탭을 눌러야 나온다(일곱째 칸 = 누를 글자).
    #
    # **상세에 무엇이 서는지는 안 박는다.** 상세는 목록 첫 줄을 따라가는데, 원천이 늘면
    # 첫 줄이 바뀐다(기반시설을 실은 날 실제로 바뀌었다). 목록에 있어야 할 것과
    # **갈래 머리글**만 본다 — 갈래가 사라지면 그건 진짜 고장이다.
    ("/buildings/1024123619", "주변 소식 · 갈래", "건물정보",
     ["주변 소식", "정비·개발", "기반시설", "규제", "정책 발표",
      "홍실아파트 주택재건축 정비사업", "25.01",
      "동부간선도로 지하화 민간투자사업 실시계획(변경) 승인"],
     "갈래가 같이 서야 한다. 날짜는 목록에서 25.01 꼴",
     ("입지", "전체")),
  ]),

  ("[정합] 상권 구성 — 업체 사전(ref.biz_category)을 읽는다", [
    # 2026-09-06 대장 용도 → 실제 업체. 삼성동 78 반경 600m 엔 일곱 갈래가 다 있다.
    # 새 갈래 셋(생활서비스·교육·의료)이 화면에 서는가만 본다 — 비율 값은 분기마다 바뀐다.
    ("/buildings/1024123619", "상권 구성 · 갈래 일곱", "건물정보",
     ["상권 구성", "업무", "먹자", "판매", "생활서비스", "교육", "유흥", "의료"],
     "새 갈래 셋이 안 서면 사전 조인이 끊긴 것이다",
     "입지"),
  ]),

  ("[정합] 토지정보 — 필지 칩은 「동 본-부번지」로 읽힌다", [
    # PNU 끝 8자리(00780000)를 세우던 것을 지번으로(2026-09-07 대표). 삼성동 140-3: 대표 + 부속 넷
    # 준비 글자는 **부속 필지 지번**으로 — 「토지정보」는 필지 조회가 오기 전에도 서 있어서 칩 없이 글자를 모았다.
    # 「삼성동 140-3번지」는 페이지 머리 주소에도 있어 칩이 없어도 맞아 버린다
    ("/buildings/1024111479", "다필지 · 지번 칩", "삼성동 140-4번지",
     ["삼성동 140-3번지", "삼성동 140-4번지", "삼성동 140-8번지", "삼성동 140-10번지"],
     "칩 문구가 PNU 숫자로 돌아가면 API 의 label 이 빠진 것이다",
     None),
  ]),

  ("[정합] 승강기 — 대장이 비면 공단 값이 옆에 참조로 선다", [
    # 0156 로 「대장이 본값」이 됐는데 참조 칸(elevator_ext)이 한 번도 안 찼다(2026-09-07 발견).
    # 값이 build_integrated 에서 끊겨 585,731동 전부 빈칸이었다. 이제 114,423동에 붙는다.
    # 역삼동 726: 대장 승용승강기수가 없고 승강기공단은 8대. 참조 줄이 서야 한다.
    ("/buildings/102417758", "승강기공단 참조", "건물정보",
     ["엘리베이터", "승강기공단"],
     "대장이 비었을 때만 옆에 선다. 본값을 덮으면 안 된다",
     None),   # 「건물 · 토지」가 기본 탭이라 누를 것이 없다
  ]),
  ...
  ("[정합] 소식 — 서울 전체를 자리 없이 읽는다", [
    # /news 는 area_event + urban_notice 를 합쳐 날짜순으로 낸다. 갈래 칩이 여섯 서고
    # 목록이 비지 않아야 한다. 값은 안 박는다 — 매주 새 고시가 위로 온다.
    ("/news", "소식 · 갈래 칩", "소식",
     ["정비·개발", "기반시설", "건축 인허가", "규제", "정책 발표", "고시·공고", "출처"],
     "여섯 갈래가 다 서고 첫 줄 상세가 그려져야 한다"),
  ]),
...
# 실행은 평평하게 — (묶음이름, 표본…)
# 표본은 여섯 칸(경로·이름·기다릴 글자·있어야 할 글자들·왜)이고,
# **일곱째 칸은 「먼저 누를 글자」**다 — 탭 안쪽 카드는 눌러야 그려진다. 없으면 안 누른다.
FIXTURES = [(g, *f, *([None] * (6 - len(f)))) for g, fs in GROUPS for f in fs]
...
async def main() -> int:
    ...
    async with async_playwright() as pw:
        ...
        print("로그인 됨\n", flush=True)
        group = None
        for grp, path, name, ready, must, why, click in FIXTURES:
            if grp != group:
                group = grp
                print(f"\n{grp}")
            try:
                ...
                # 일곱째 칸은 글자 하나 또는 차례로 누를 글자들(탭 → 칩). 주변 소식은 기본 1년이라 「전체」를 눌러야 옛 줄이 선다
                for ck in ([click] if isinstance(click, str) else (click or [])):
                    await pg.get_by_role("button", name=ck, exact=True).first.click()
                    await pg.wait_for_timeout(1200)     # 탭 갈아 끼우고 조회가 돌아올 틈
                if path == "/search":
