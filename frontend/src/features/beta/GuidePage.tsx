...
type Slide = { t: string; img?: string; steps?: string[]; rows?: [string, string][]; note?: { t: string; d: string } };
...
const SECTIONS: Sec[] = [
  {
    ...
    slides: [
      { t: "검색창에 입력", img: "01-검색.png",
        steps: [
          "주소 · 건물명 · 역 이름을 입력합니다. 띄어쓰기는 없어도 되고 지번 일부만 쳐도 됩니다(예: 신사동 561).",
          "건물을 고르면 지도가 이동하며 왼쪽에 요약 카드가, 지역·역을 고르면 지도만 이동합니다.",
          "요약 카드의 「상세보기 →」를 누르면 새 탭에서 상세 화면이 열립니다.",
        ],
        note: { t: "'번지'는 빼고 입력", d: "역삼동 659번지 ✗ · 역삼동 659 ✓" } },
      { t: "지도 도구", rows: [
        ["거리재기", "두 지점을 클릭해 직선 거리 측정"],
        ["면적 · 반경", "임의 구역의 면적, 특정 지점 기준 반경 원"],
        ["지적도", "필지 경계선을 지도에 겹쳐 표시"],
        ["로드뷰", "거리 사진 — 건물 외관·주변 환경 확인"],
        ["일반 / 위성", "지도 배경 전환"],
      ] },
    ],
  },
  {
    ...
    slides: [
      ...
      { t: "카테고리별 조건", img: "11-필터카테고리.png", rows: [
  ...
  },
  {
    ...
    slides: [
      ...
      { t: "검색 결과 읽기", img: "10-검색결과.png", rows: [
        ...
        ["카드의 가격", "팀이 매매가를 안 적었으면 빌탐정 적정가가 표시됩니다"],
        ["지도 핀", "검은 원 안 숫자 = 그 구역의 건물 수. 확대하면 개별 핀으로 나뉩니다"],
      ] },
    ],
  },
  {
    id: "detail", no: "04", t: "매물 상세 읽기", sub: "리포트 요약",
    lead: "건물 하나의 결론이 한 화면에.",
    slides: [
      { t: "요약 화면", img: "02-리포트요약.png",
        steps: [
          "상단 「표시 범위」에서 리포트 요약 / 전체 / 매물 / 건물·토지를 골라 보고 싶은 만큼만 봅니다.",
          "요약은 적정가·수익률·매력도·투자유형·미래가치를 한 장에 담아 고객 앞에서 바로 열 수 있습니다.",
        ] },
      { t: "지표 읽는 법", rows: [
        ["빌탐정 적정가", "주변 실거래를 토지·건물 가치로 나눠 비교하고 연식·거래시점·규모를 보정한 값. 상업·업무용만 산출"],
        ["가격 비교", "적정가 · 최근 실거래 · 공시지가 총액을 막대로 나란히"],
        ["예상수익률", "연 임대수익 ÷ 매매가. 주변 평균과 비교됩니다"],
        ["매력도", "S~D 등급 + 8축 레이더(도로접면·역거리·용도지역·형상·승인일·엘리베이터·대수선·지세)"],
        ["투자 유형", "신축용/리모델링용/수익형 적합도 + 사옥 적합 + 주변 상권 지도"],
        ["미래가치", "개발여지(용적 여유)·정비구역 지정·임대 상향 여력·지가 5년 추세로 판정"],
      ],
        note: { t: "가격 3층 구조", d: "매도희망가(건물주) · 매매가(중개인 판단, 비우면 적정가로 대체) · 빌탐정 적정가(시스템). 매도희망가와 매매가를 함께 적으면 협의 필요금액이 자동 계산됩니다." } },
    ],
  },
  {
    ...
    slides: [
      { t: "값 수정", img: "14-값수정.png",
        ...
        note: { t: "수정은 우리 팀에게만", d: "공공데이터 원본은 그대로 남고 팀 화면에만 덮어써집니다. 고친 값은 적정가·평단가·수익률 계산에도 즉시 반영됩니다." } },
      { t: "층별 임대 입력", img: "15-층별임대.png",
        steps: [
  ...
  },
  {
    ...
    lead: "적정가와 주변시세는 '주변'의 범위로 달라집니다.",
    slides: [
      { t: "직접 그리기", img: "03-상권정의.png",
        steps: [
...
const FAQ: [string, string][] = [
  ["적정가가 시세와 다르다", "주변 사례 기반 추정이라 개별 사정(급매·내부 상태)은 반영되지 않습니다. 검토 화면에서 어떤 사례가 쓰였는지 확인하고 직접 조정하세요."],
  ["수익률이 5% 이상", "서울 상업용은 보통 2~4%대입니다. 임대 추정이 높게 잡혔을 가능성이 크니 층별 임대를 입력하면 정확해집니다."],
  ["적정가가 안 나온다", "주거용이거나 주변 비교 거래가 3건 미만입니다. 상권 범위를 넓혀보세요."],
  ["크레딧", "검색·상세·검토는 무료. 리포트 생성만 30 크레딧이며 가입 시 60(2건)이 지급됩니다."],
];
  ...
  return (
    <div className="bt-beta">
      ...
      <main className="bt-main">
        ...
        <section className="gsec" id="faq">
          <div className="gsec-h"><span className="no">08</span><h2>알아두기</h2></div>
          <dl className="grows plain">
            {FAQ.map(([k, v]) => (<div key={k}><dt>{k}</dt><dd>{v}</dd></div>))}
          </dl>
        </section>
