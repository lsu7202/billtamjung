    ...
    <div ref={scRef} onScroll={onScroll} className="story-root">
      ...
      <section data-i={1} ref={setRef(1)} className={cls(1, true)}>
        <div style={{ display: "flex", gap: "4vw", alignItems: "center", width: "100%" }}>
          <div style={{ flex: 1 }}>
            ...
            <h2 className="story-h">{addr}의 <b>빌탐정 추정가</b></h2>
      ...
      <section data-i={4} ref={setRef(4)} className={cls(4)}>
        ...
        <table className="story-tbl">
          ...
          <tbody>
            ...
            )) : <tr><td colSpan={8} style={{ textAlign: "center", color: "#8B95A1", padding: 16 }}>반경 내 실거래 사례 없음</td></tr>}
            {moreCount > 0 && <tr><td colSpan={8} style={{ textAlign: "center", color: "#8B95A1", fontSize: 13, padding: 8, borderTop: "1px dashed #E5E8EB" }}>가까운 순 4건 표시 · 외 <b style={{ color: "#191F28" }}>+{moreCount}건</b>도 추정가 산정에 반영됨</td></tr>}
          </tbody>
        </table>
        <div className="st-cols" style={{ marginTop: "2.5vh", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: "clamp(13px,1.3vw,16px)", fontWeight: 700, color: "#191F28", marginBottom: 8 }}>연면적당 평단가 비교 <span style={{ color: "#8B95A1", fontWeight: 400 }}>(만원/평)</span></div>
            {comps.length >= 2 && <CompareBar height={140} fmt={(v) => `${Math.round(v / 1e4).toLocaleString()}`}
              ...
              items={[...comps.map((c, i) => ({ label: `${i + 1}`, value: c.per_now ?? 0, color: "#191F28" })),
                ...(avgPer ? [{ label: "본매물", value: avgPer, color: "#3182F6", strong: true }] : [])].filter((x) => x.value > 0)} />}
          </div>
          <div style={{ alignSelf: "center" }}>
            <p style={{ fontSize: "clamp(14px,1.45vw,18px)", lineHeight: 1.7, color: "#191F28" }}>
              {compMin && compMax
                ? <>인근 유사 실거래의 연면적당 평단가는 사례별 약 <b>{compMin.toLocaleString()}~{compMax.toLocaleString()}만원</b> 수준입니다. 본 매물은 입지·용도·건물 규모 등 개별 특성을 반영해 <b style={{ color: "#3182F6" }}>평당 약 {avgPer ? Math.round(avgPer / 1e4).toLocaleString() : "—"}만원</b> 수준으로 분석됩니다.</>
      ...
      <section data-i={5} ref={setRef(5)} className={cls(5)}>
        ...
        <div className="story-row" style={{ marginTop: "2vh" }}>
          {gongsiMetrics.map((g) => (
            <div key={g.k} className="story-cell">
              ...
              <div className="v" style={{ color: "#3182F6" }}>{g.v}</div>
        ...
        <div className="st-cols" style={{ marginTop: "3vh" }}>
          <div>
            <div style={{ fontSize: "clamp(14px,1.4vw,17px)", fontWeight: 700, color: "#191F28", marginBottom: 10 }}>최근 공시지가 비교 <span style={{ color: "#8B95A1", fontWeight: 400 }}>(만원/㎡)</span></div>
            {gLatest && nbhdGongsi
              ? <CompareBar height={160} fmt={(v) => `${Math.round(v / 1e4).toLocaleString()}`}
                  items={[{ label: "본매물", value: gLatest, color: "#3182F6", strong: true }, { label: "주변 평균", value: nbhdGongsi, color: "#191F28" }]} />
              : <p className="story-sub">주변 사례 공시지가 데이터가 부족합니다.</p>}
          </div>
          <div style={{ alignSelf: "center" }}>
            <div style={{ fontSize: 14, color: "#8B95A1" }}>공시총액 (공시지가 × 대지면적)</div>
            <div className="st-lead" style={{ fontSize: "clamp(26px,3.4vw,46px)" }}>{gTotal ? eokman(gTotal) : "—"}</div>
            <p style={{ fontSize: "clamp(14px,1.4vw,17px)", lineHeight: 1.7, color: "#191F28", marginTop: 12 }}><Prose segs={gongsiProse} bold="#3182F6" /></p>
          </div>
        </div>
      </section>
      ...
      <section data-i={6} ref={setRef(6)} className={cls(6)}>
        ...
        <div style={{ display: "flex", gap: "2vw", marginTop: "2vh" }}>
          {rentMetrics.map(([k, v]) => (
            <div key={k} style={{ flex: 1, borderTop: "2px solid #191F28", paddingTop: 10 }}>
              <div style={{ fontSize: "clamp(12px,1.2vw,14px)", fontWeight: 700, color: "#191F28" }}>{k}</div>
              <div style={{ fontSize: "clamp(19px,2.2vw,28px)", fontWeight: 800, color: "#191F28", lineHeight: 1.1, marginTop: 2 }}>{v}</div>
            </div>
          ))}
        </div>
        <div className="st-cols" style={{ marginTop: "3.5vh" }}>
          <div style={{ display: "flex", gap: "3vw" }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "clamp(13px,1.3vw,16px)", fontWeight: 700, color: "#191F28", marginBottom: 8 }}>현재 vs 주변 임대시세 <span style={{ color: "#8B95A1", fontWeight: 400 }}>(월, 만원)</span></div>
              {curRent && rent
                ? <CompareBar height={140} fmt={(v) => `${Math.round(v / 1e4).toLocaleString()}`}
                    items={[{ label: "현재", value: curRent, color: "#191F28" }, { label: "주변시세", value: rent, color: "#3182F6", strong: true }]} />
                : <p className="story-sub">주변 임대사례가 부족합니다.</p>}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: "clamp(13px,1.3vw,16px)", fontWeight: 700, color: "#191F28", marginBottom: 8 }}>수익률 vs 주변 평균 <span style={{ color: "#8B95A1", fontWeight: 400 }}>(%)</span></div>
              {roiFair != null && nbhdRoi != null
                ? <CompareBar height={140} fmt={(v) => v.toFixed(2)}
                    items={[{ label: "본매물", value: roiFair, color: "#3182F6", strong: true }, { label: "주변평균", value: nbhdRoi, color: "#191F28" }]} />
                : <p className="story-sub">주변 수익률 데이터가 부족합니다.</p>}
            </div>
          </div>
          <div style={{ alignSelf: "center" }}>
            <p style={{ fontSize: "clamp(15px,1.5vw,18px)", lineHeight: 1.7, color: "#191F28" }}><Prose segs={rentProse} bold="#3182F6" /></p>
          </div>
        </div>
      </section>
