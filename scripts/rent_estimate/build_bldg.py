...
import asyncpg
from rent_common import EXCL, M2P, rate_for, pick_rate, market_adj, SANG_SQL, BLDG_FILTER, pick_series, DEPOSIT_MULT
...
async def main():
    ...
    fo = await c.fetch(
        f"SELECT fo.building_pk pk, fo.floor, fo.use, fo.exclusive_area a "
    ...
    for b in bldgs:
        ...
        wol = rentable = 0
        for fl, a in flrs:
            rt = pick_rate(series, fl, rate)
            if rt:
                wol += rt * 1000 * a * market_adj(fl)
                rentable += a
        if wol <= 0 or rentable <= 0:
            continue
        py = rentable / M2P
        ins.append((b['pk'], b['sang'] if mapped else None, series, int(wol), int(wol * 12),
                    int(wol / py) if py else 0, int(wol * DEPOSIT_MULT), round(rentable, 1), mapped))
        if mapped:
            n_map += 1
    await c.executemany(
