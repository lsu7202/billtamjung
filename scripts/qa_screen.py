...
def check_shape(text: str):
    ...
    for label, pat, why in LINE_SHAPE:
        ...
        got = "\n".join(rest.split("\n")[:3])
        if not re.match(pat, got):
            out.append((label, got.replace("\n", " "), why))
    return out

# ── ② 표본마다 반드시 떠야 할 글자 ───────────────────────────────
# (경로, 이름, [반드시 있어야 할 조각], 왜 이 표본인가)
FIXTURES = [
    ("/buildings/1024123619", "강남 삼성동 78",
     ["55%", "508%"],
     "값이 하나인 걸침 필지. 토지이음 화면과 같은 값이어야 한다"),

    ("/buildings/1024114645", "강남 · 병기 필지",
     ["50%, 60%", "100%, 150%"],
     "값이 둘인 필지. 하나만 고르거나 '50, 60%%' 로 뭉개지면 안 된다"),

    ("/buildings/101815037", "여러 필지 · 400 vs 2635",
     ["2635%"],
     "건물이 값 다른 필지에 걸쳐 있다. 텍스트로 비교하면 '400%'>'2635%' 라 400을 고른다"),

    ("/buildings/1024118652", "강남 · 건폐 110%",
     ["110%", "300%"],
...
async def main() -> int:
    ...
    async with async_playwright() as pw:
        ...
        if not await _login(pg, a.base, email, pw_):
            ...
            return 2
        print("로그인 됨\n", flush=True)
        for path, name, must, why in FIXTURES:
            try:
                await pg.goto(a.base + path, wait_until="domcontentloaded")
                # 「법정 건폐」 줄이 그려질 때까지 기다린다 — 정해진 시간만 쉬면 덜 그려진 화면을 본다
                await pg.wait_for_function(
                    "() => document.body.innerText.includes('법정 건폐')", timeout=a.wait)
