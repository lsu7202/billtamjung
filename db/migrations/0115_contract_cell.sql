-- 「매칭」을 **계약**으로(2026-08-19) — 부동산에서 안 쓰는 낱말이라 뜻이 안 통했다.
--   칸의 뜻도 좁힌다: 상대만 골라선 초록이 아니다. **누구와 · 얼마에**가 둘 다 서야 계약이다.
--     회색  담은 매수자가 없다
--     노랑  담았거나, 상대는 골랐는데 값이 비었다(부분 채움)
--     초록  picked_at + deal_price
BEGIN;
  ...
  SELECT
    ...
    (EXISTS (SELECT 1 FROM app.proposals pr
              WHERE pr.team_id = l.team_id AND pr.building_pk = l.building_pk
                AND pr.picked_at IS NOT NULL AND pr.deal_price IS NOT NULL))   AS s6_match,
    (EXISTS (SELECT 1 FROM app.proposals pr
              WHERE pr.team_id = l.team_id AND pr.building_pk = l.building_pk
...
COMMENT ON VIEW app.v_listing_stage IS
  '매물 사다리 — 필드 파생(S04b §2.1). 다섯째 칸 = 계약(상대+값, 0113~0115).';
  ...
  SELECT b.id, b.team_id,
    ...
    EXISTS (SELECT 1 FROM app.proposals p
             WHERE p.buyer_id = b.id
               AND p.picked_at IS NOT NULL AND p.deal_price IS NOT NULL)         AS b4_match,
    EXISTS (SELECT 1 FROM app.proposals p
...
INSERT INTO app.schema_migrations(version) VALUES ('0115_contract_cell.sql')
